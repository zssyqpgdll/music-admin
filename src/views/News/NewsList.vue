<template>
  <div>
    ...
    <div class="header-menu">
      ...
      <el-button type="primary" @click="addStudent()" class="addbtn">添加资讯</el-button>
...
<!--      <el-table-column prop="classNo" label="班级" fixed width="100"></el-table-column>-->
      ...
      <el-table-column prop="type" label="类型" >
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type==1" plain >养殖技术</el-tag>
          <el-tag v-else-if="scope.row.type==2" plain>利润技巧</el-tag>
          <el-tag v-else-if="scope.row.type==3" plain>品种选育</el-tag>
          <el-tag v-else-if="scope.row.type==4" plain>业界要闻</el-tag>
        </template>
      </el-table-column>
...
<!--        </el-form-item>-->
    ...
    <el-dialog title="添加资讯" :visible.sync="visibleAddForm">
      <el-form
              :model="addFormdata"
              label-position="left"
              label-width="80px"
...
<script>
  import {add,del,modify,get,search2} from "@/api/news";
  ...
  methods: {
    ...
    handleDownload() {
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['ID','标题' ,'内容', '点赞数', '收藏数', '作者','类型']
        const filterVal = ['id','title', 'content','thumb' ,'star', 'author', 'type']
