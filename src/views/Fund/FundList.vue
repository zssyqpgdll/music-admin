...
<!--      <el-button type="primary" @click="addStudent()" class="addbtn">添加</el-button>-->
    ...
    <el-table :data="studentData" size="mini" :stripe="true" :highlight-current-row="true">
      ...
      <el-table-column prop="status" label="支出" :formatter="shouzhi"></el-table-column>
  ...
  methods: {
    ...
    handleDownload() {
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['ID','工会' ,'经费内容', '类型', '支出','金额', '时间']
        const filterVal = ['id','groupId', 'reason', 'type', 'status', 'amount','createTime']
